 ...
 *   - names one concrete artifact (compliance matrix, shred, Section L),
 *   - carries real social proof (SENDER.userCount / SENDER.pipeline),
 ...
 * than five rewordings of one pitch.
 *
 ...
 *
 * Client-safe: imports nothing but ./sender.
 */
...
export const TEMPLATE_LIBRARY: readonly TemplateVariant[] = [
  {
    key: 'process',
    ...
    body: [
      ...
      '',
      `I ask because I work on ${SENDER.product}. Contractors use it to shred a solicitation into a compliance matrix and get a first draft back the same day, in their voice, on their strategy. ${SENDER.userCount} run on it now, across ${SENDER.pipeline}.`,
      '',
    ...
    followupBody: [
      'Hi,',
      '',
      `One more note and then I will leave it alone. The firms getting the most out of ${SENDER.product} are the ones bidding more than they can comfortably write.`,
      '',
  ...
  },
  {
    ...
    body: [
      ...
      '',
      `That gap is the reason ${SENDER.product} exists. It shreds the solicitation, builds the compliance matrix, and hands back a first draft, so a thin week stops deciding what you bid. ${SENDER.userCount} use it, across ${SENDER.pipeline}.`,
      '',
    ...
    followupBody: [
      'Hi,',
      '',
      `Short version of my last note: ${SENDER.product} takes the shred and the first draft, so a thin week stops deciding what you bid.`,
      '',
  ...
  },
  {
    ...
    body: [
      ...
      '',
      `Asking because it is the piece contractors tell me eats the most time on a deadline: hunting the right write up, then reshaping it for this Section L. ${SENDER.product} keeps it organized and pulls the matching one into the draft. ${SENDER.userCount} use it, across ${SENDER.pipeline}.`,
      '',
  ...
  },
  {
    ...
    body: [
      ...
      '',
      `If so, that is usually where the crunch shows. ${SENDER.product} shreds the solicitation into a compliance matrix and returns a first draft the same day, so the team spends its hours on win themes instead of formatting. ${SENDER.userCount} use it now, across ${SENDER.pipeline}.`,
      '',
    ...
    followupBody: [
      'Hi,',
      '',
      `Adding one thing to my last note: on a recompete the crunch is rarely the strategy, it is the production. That is the part ${SENDER.product} takes.`,
      '',
  ...
  },
  {
    ...
    body: [
      ...
      '',
      `Happy to be pointed elsewhere if it is not you. The reason I ask: I work on ${SENDER.product}, which ${userCountMid} use to turn a solicitation into a compliance matrix and a first draft the same day, across ${SENDER.pipeline}.`,
      '',
