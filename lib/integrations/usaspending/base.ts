 ...
 * the USASpending public API. Never throws — always returns a typed result so
 * callers can fall back without wrapping in try/catch.
 *
 * Ported from SourceGent's src/lib/market-check/clients/base.ts (see
 * docs/specs/sourcegent-reference/market-check-base.ts). Verbatim except for
 * this header — the contract is generic and needs no Signalgent-specific change.
 *
 ...
 * external API outages as data states (not exceptions) keeps callers linear.
 */
