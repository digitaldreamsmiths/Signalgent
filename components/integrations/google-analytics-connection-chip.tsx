...
/**
 * Google Analytics connection chip + detail popover.
 *
 * Mirrors components/integrations/gmail-connection-chip.tsx — same three
 * states (not_connected / connected / error), same popover layout. Kept
 * as its own component rather than a generic chip because the label,
 * connect URL hook, and disconnect action differ per service; a generic
 * chip would end up being a soup of props.
 */
...
import { disconnectGoogleAnalytics } from '@/lib/integrations/actions'
...
export function GoogleAnalyticsConnectionChip() {
  const { activeCompany } = useCompany()
  const companyId = activeCompany?.id ?? null
  const { status, isLoading, refresh } = useGoogleAnalyticsConnectionStatus(companyId)
  ...
  if (status.status === 'not_connected') {
    return (
      <a
        href={getGoogleAnalyticsConnectUrl(companyId)}
        style={{
          fontSize: 11,
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      >
        ...
        />
        {isHealthy ? 'Analytics connected' : `Analytics ${status.status}`}
      </button>
      ...
      {open && (
        ...
        >
          ...
          <Row label="Property" value={status.accountLabel ?? '—'} />
