...
/**
 * Pinterest connection chip + detail popover.
 *
 * Mirrors components/integrations/linkedin-connection-chip.tsx — same
 * three-state shape, same popover layout, same marketing-mode coral.
 * Both chips can render side-by-side on /marketing.
 */

import { useEffect, useRef, useState, useTransition } from 'react'
import { useCompany } from '@/contexts/company-context'
import {
  usePinterestConnectionStatus,
  getPinterestConnectUrl,
} from '@/hooks/use-pinterest-connection'
import { disconnectPinterest } from '@/lib/integrations/actions'
...
export function PinterestConnectionChip() {
  const { activeCompany } = useCompany()
  const companyId = activeCompany?.id ?? null
  const { status, isLoading, refresh } = usePinterestConnectionStatus(companyId)
  ...
  function handleDisconnect() {
    ...
    startTransition(async () => {
      await disconnectPinterest(companyId)
      await refresh()
      setOpen(false)
    })
  }
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      >
        ...
        />
        {isHealthy ? 'Pinterest connected' : `Pinterest ${status.status}`}
      </button>
      ...
      {open && (
        ...
        >
          ...
          <Row label="Account" value={status.accountLabel ? `@${status.accountLabel}` : '—'} />
