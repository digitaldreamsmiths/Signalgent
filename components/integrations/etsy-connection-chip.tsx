...
/**
 * Etsy connection chip + detail popover.
 *
 * Mirrors components/integrations/google-analytics-connection-chip.tsx
 * — same three states (not_connected / connected / error), same
 * popover layout. Kept as its own component rather than a generic chip
 * because the label, connect URL hook, and disconnect action differ
 * per service.
 */
...
import { disconnectEtsy } from '@/lib/integrations/actions'
...
export function EtsyConnectionChip() {
  const { activeCompany } = useCompany()
  const companyId = activeCompany?.id ?? null
  const { status, isLoading, refresh } = useEtsyConnectionStatus(companyId)
  ...
  if (status.status === 'not_connected') {
    return (
      <a
        href={getEtsyConnectUrl(companyId)}
        style={{
          fontSize: 11,
  ...
  function handleDisconnect() {
    ...
    startTransition(async () => {
      await disconnectEtsy(companyId)
      await refresh()
      setOpen(false)
    })
  }
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      >
        ...
        />
        {isHealthy ? 'Etsy connected' : `Etsy ${status.status}`}
      </button>
      ...
      {open && (
        ...
        >
          ...
          <Row label="Shop" value={status.accountLabel ?? '—'} />
