...
/**
 * Stripe connection chip + detail popover.
 *
 * Three visual states driven by useStripeConnectionStatus:
 *   1. not_connected  → "Connect Stripe" button, navigates to OAuth connect route
 *   2. connected      → "Stripe connected" pill, click opens detail popover
 *   3. error/expired/revoked/disconnected → warning pill, click opens popover
 *                       with the error message and a reconnect CTA
 *
 * Popover contents: provider name, account label, connected date, last sync,
 * status, Disconnect button. Click outside closes.
 */
...
import { disconnectStripe } from '@/lib/integrations/actions'
...
export function StripeConnectionChip() {
  const { activeCompany } = useCompany()
  const companyId = activeCompany?.id ?? null
  const { status, isLoading, refresh } = useStripeConnectionStatus(companyId)
  ...
  // State 1: not connected → plain CTA link
  ...
  function handleDisconnect() {
    ...
    startTransition(async () => {
      await disconnectStripe(companyId)
      await refresh()
      setOpen(false)
    })
  }
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      >
        ...
        />
        {isHealthy ? 'Stripe connected' : `Stripe ${status.status}`}
      </button>
