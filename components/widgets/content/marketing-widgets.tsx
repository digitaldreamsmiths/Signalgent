    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
      {kpis.map((k) => (
        <div key={k.label} style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 500, color: '#b4441e' }}>{k.value}</div>
        </div>
      ))}
    </div>
  )
        ...
        {DAYS.map((day, i) => (
          <div key={day}>
            ...
            <div style={{ minHeight: 50, background: 'rgba(255,255,255,0.015)', border: '1px solid #272727', borderRadius: 5, padding: 5, display: 'flex', flexDirection: 'column', gap: 3 }}>
              {posts[i]?.map((p) => (
                <span key={p} style={{ fontSize: 9, color: '#b4441e', background: 'rgba(216,90,48,0.15)', borderRadius: 3, padding: '2px 5px' }}>{p}</span>
              ))}
            </div>
          </div>
        ))}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {m.recentPosts.slice(0, 5).map((post, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid #272727' }}>
          ...
          <span style={{ fontSize: 9, color: post.status === 'Published' ? '#6a6' : post.status === 'Scheduled' ? '#b4441e' : '#999999', flexShrink: 0 }}>{post.status}</span>
  ...
  return (
    <ResponsiveContainer width="100%" height={160}>
      <LineChart data={data}>
        ...
        <Tooltip contentStyle={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 6, fontSize: 11 }} />
        <Line type="monotone" dataKey="rate" stroke="#b4441e" strokeWidth={2} dot={{ fill: '#b4441e', r: 3 }} animationDuration={400} />
      </LineChart>
    </ResponsiveContainer>
  )
}
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 18, fontWeight: 500, color: '#b4441e' }}>{m.topPost.reach.toLocaleString()}</div>
        </div>
        <div>
          <div style={{ fontSize: 9, color: '#666666' }}>Engagement</div>
          <div style={{ fontSize: 18, fontWeight: 500, color: '#b4441e' }}>{m.topPost.engagement}</div>
        </div>
      </div>
    </div>
  )
    ...
    <ResponsiveContainer width="100%" height={160}>
      <BarChart data={data}>
        ...
        <Bar dataKey="posts" fill="#712B13" radius={[3, 3, 0, 0]} animationDuration={400}>
          {data.map((_, i) => <Cell key={i} fill={i === data.length - 1 ? '#b4441e' : '#712B13'} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  )
